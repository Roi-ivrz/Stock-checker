...
class mainObj:
    ...
    def find_anomalies(self, data):
        ...
        return d, data_mean, data_std

    def customPrint(self, d, tick, data_mean, data_std):
        print("\n\n\n*******  " + tick.upper() + "  *******")
        print("Ticker is: "+tick.upper())
        print('mean: {}'.format(data_mean) + ' std: {}'.format(data_std))
    ...
    def parallel_wrapper(self, x, currentDate, positive_scans):
        global DAY_CUTTOFF
        d, data_mean, data_std = (self.find_anomalies(self.getData(x)))
        if d['Dates']:
            for i in range(len(d['Dates'])):
                if self.days_between(str(currentDate)[:-9], str(d['Dates'][i])) <= DAY_CUTTOFF:
                    self.customPrint(d, x, data_mean, data_std)
                    stonk = dict()
                    stonk['Ticker'] = x
    ...
    def main_func(self):
        ...
        list_of_tickers = StocksController.get_list()
